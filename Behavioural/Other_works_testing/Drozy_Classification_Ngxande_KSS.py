...
#######################################################################################################################
# Code to test Ngxande method with KSS labelling
#######################################################################################################################
...
sequence_length = 10
case = 1  # 1 for 789 drowsy, 2 for 6 removed and 789 drowsy and 3 for first vs last
...
a_data, a_sub_num, a, d_data, d_sub_num, d, get_train_test_Behavioural, Awake_training, Drowsy_training, \
a_sub_trials, d_sub_trials = get_data(case, data, KSS, subs_data)

tr1_data = a_data.tolist()
tr2_data = d_data.tolist()

model_data = np.concatenate([tr2_data, tr1_data], axis=0)
model_labels = np.concatenate([d, a], axis=0)
